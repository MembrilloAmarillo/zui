...
pub const events = enum(u32) { NoEvent, LeftClick, RightClick, MiddleClick, Delete, ScrollUp, ScrollDown, Enter, Quit };

pub const event_output = struct { event: events, key: u8 };
...
pub const context = struct {
    ...
    pub fn init(get_event: *const fn () event_output) context {
        return .{ .input = events.NoEvent, .cursor_pos = .{ .x = 0, .y = 0 }, .layouts = layout_stack.init(), .boxes = box_stack.init(), .get_event = get_event, .parents = parent_stack.init(), .input_idx = 0, .focus_box = undefined, .current_event = event_output{ .event = events.NoEvent, .key = 0 } };
    }
    ...
    pub fn window_begin(self: *context, id: []const u8, win_size: rect, options: layout_options) bool {
        const new_box = box{ .id = id, .bounds = win_size, .color = @Vector(4, f32){ 0.15, 0.15, 0.15, 1.0 }, .options = options };
    ...
    pub fn get_last_box(self: *context) *box {
        return self.boxes.get_front();
    }
    ...
    pub fn textbox(self: *context, id: []u8, bound: rect, color: @Vector(4, f32), options: layout_options) events {
        ...
        if (self.is_cursor_over(new_box.bounds) or self.boxes.get_front() == self.focus_box) {
            ...
            if (self.current_event.key > 255) {} else if (@as(u8, @intCast(self.current_event.key)) >= ' ' and @as(u8, @intCast(self.current_event.key)) <= '~') {
    ...
    pub fn checkbox(self: *context, id: []const u8, bound: rect, color: @Vector(4, f32), check_confirm: *bool) events {
        ...
        if (self.is_cursor_over(new_box.bounds) or check_confirm.*) {
            ...
            if (self.is_cursor_over(new_box.bounds) and self.current_event.event == events.LeftClick) {
    ...
    pub fn begin_scrollview(self: *context) void {
        const new_layout = layout{ .bounds = self.boxes.get_front().bounds, .content = rect.init(0, 0, 0, 0), .default_options = layout_options{ .DRAW_RECT = true, .DRAW_BORDER = true } };
